...
namespace FOOTBALL
{
    public partial class ScatterPlot : AnimatedControl
    {
        ...
        public void Feed()
        {
            xave = m_Pairs.XCoords.Average();
            yave = m_Pairs.YCoords.Average();
        ...
        protected override void OnPaintBackground(PaintEventArgs pe)
        {
            ...
            if (m_Pairs != null && m_Pairs.Count > 0)
            {
                ...
                {
                    int x = (int)(scoot*((m_Graph.Image.Width) * (m_Pairs.XCoords[k] - xminVal) / (xmaxVal-xminVal)));
                    int y = (int)((m_Graph.Image.Height)*(1 - (m_Pairs.YCoords[k] - yminVal) / (ymaxVal- yminVal)));
                        m_Graph.Graphics.FillEllipse(m_DataBrush, x - DotRadius, y - DotRadius, 2 * DotRadius, 2 * DotRadius);
                }
                if(SelectedIndex >= 0)
                {
                    int x = (int)(scoot * ((m_Graph.Image.Width) * (m_Pairs.XCoords[SelectedIndex] - xminVal) / (xmaxVal - xminVal)));
                    int y = (int)((m_Graph.Image.Height) * (1 - (m_Pairs.YCoords[SelectedIndex]-yminVal) / (ymaxVal - yminVal)));
                ...
                if (SelectedIndex2 >= 0)
                {
                    int x = (int)(scoot * ((m_Graph.Image.Width) * (m_Pairs.XCoords[SelectedIndex2] - xminVal) / (xmaxVal - xminVal)));
                    int y = (int)((m_Graph.Image.Height) * (1 - (m_Pairs.YCoords[SelectedIndex2] - yminVal) / (ymaxVal - yminVal)));
